  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        ...
        align={align}
        className={cn(
          "z-50 max-h-(--radix-dropdown-menu-content-available-height) w-(--radix-dropdown-menu-trigger-width) min-w-32 origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-lg bg-popover p-1 text-popover-foreground shadow-xl ring-1 ring-foreground/10 duration-200 transition-all data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className,
        )}
        {...props}
      />
...
function DropdownMenuItem({ className, inset, variant = "default", ...props }) {
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        "group/dropdown-menu-item relative flex min-h-8 cursor-pointer items-center gap-3 rounded-md px-3 py-2 text-xs outline-hidden select-none transition-all duration-300",

        // ÖZEL HOVER EFEKTİ:
        // Arka planı belirginleştirip hafif bir "iç gölge" ve "yükselme" efekti ekliyoruz
        "hover:bg-indigo-600 hover:text-white hover:shadow-md hover:shadow-indigo-500/30 hover:translate-x-0.5",

        // Odaklanma (Focus) durumu
        "focus:bg-indigo-600 focus:text-white",

        // Diğer standartlar
        "data-inset:pl-7.5 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
